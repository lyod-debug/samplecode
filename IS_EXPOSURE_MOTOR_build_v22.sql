/* =====================================================================
   IS_EXPOSURE_MOTOR — v22
   REAL BUG FIX: COVERAGE_PRIORITY referenced VEC_GMP_COVERAGEID, which
   does not exist - confirmed against real IS_COVERAGE data that the
   actual column is VEC_GWP_COVERAGEID (GWP, not GMP). Fixed in all 5
   occurrences (4 blocks + 1 validation query). This was a plain
   typo/misremembered name, not a design error - Subtype='VehicleCoverage'
   was independently confirmed correct against real IS_COVERAGE_MOTOR
   data and needed no change.
   STILL OPEN, NOT YET RESOLVED: whether IS_COVERAGE_MOTOR (or the
   consolidated IS_COVERAGE) actually carries coverage-origin rows for
   the 5 core CT codes exposure needs, or whether motor's real purchased
   coverages load some other way entirely. Waiting on the 3 verification
   queries from chat before treating COVERAGE_PRIORITY's coverage-vs-
   inclusion priority logic as fully confirmed - the typo fix alone
   makes it RUN, not necessarily makes it CORRECT.
   ===================================================================== */
/* =====================================================================
   IS_EXPOSURE_MOTOR — v22
   REAL BUG FIX #1: COVERAGE_PRIORITY referenced VEC_GMP_COVERAGEID,
   which does not exist - confirmed against real IS_COVERAGE data the
   actual column is VEC_GWP_COVERAGEID (GWP, not GMP). Fixed in all 5
   occurrences. Plain typo, not a design error.
   REAL BUG FIX #2, more important: removed the coverage-vs-inclusion
   PRIORITY mechanism entirely, per your direct instruction. Nothing in
   any mapping doc ever specified preferring one origin over the other -
   that was logic I invented myself to solve a duplicate-Type scenario I
   never actually confirmed exists in real data. Renamed COVERAGE_PRIORITY
   to COVERAGE_LOOKUP - now a plain match on ClaimPublicID+Type, whichever
   origin (coverage or inclusion) has the value. The "resolved via"
   validation query is removed since there's no longer a preference to
   track; the duplicate-Type check remains, since that's still worth
   knowing about regardless of origin.
   ===================================================================== */
/* =====================================================================
...
COVERAGE_LOOKUP AS (
    -- FIXED per your instruction: no priority, no origin preference.
    -- A coverage row (VEC_GWP_COVERAGEID populated) or an inclusion row
    -- (INC_ID populated) are equally valid - whichever one exists for
    -- this claim+type is the answer, nothing in the mapping ever said
    -- to prefer one over the other. Plain match, no ROW_NUMBER ranking.
...
-- REMOVED: the "resolved via coverage vs inclusion" tracking query no
-- longer applies - there's no priority mechanism distinguishing the two
-- anymore, per your instruction. Duplicate-Type check remains below,
-- since that still matters regardless of origin preference.
